...
import { Heart, ChevronUp, ChevronDown, CheckCircle2, XCircle, AlertCircle, GraduationCap, ExternalLink } from 'lucide-react'
      ...
      <div className="flex shrink-0 flex-col items-end gap-1">
        ...
        {course.link_oficial && (
          <a
            ...
            className="mt-0.5 flex items-center gap-1 text-[10px] text-navy hover:underline"
          >
        ...
        <div className="flex items-center gap-0.5">
          <button
            onClick={onMoveUp}
            disabled={index === 0}
            className="flex h-6 w-6 items-center justify-center rounded text-muted-foreground hover:text-foreground hover:bg-muted/50 disabled:opacity-30 transition-colors"
          ...
          >
            <Heart className={cn('h-3.5 w-3.5', isCandidatura ? 'fill-destructive text-destructive' : '')} />
          </button>
        </div>
      </div>
    </div>

  ...
  return (
    <Dialog open={!!course} onOpenChange={open => { if (!open) onClose() }}>
      <DialogContent className="max-h-[85vh] max-w-xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-base text-foreground">{course.name}</DialogTitle>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-wrap gap-1.5">
            ...
            <Badge variant="outline" className="gap-1 text-[10px]"><MapPin className="h-2.5 w-2.5" />{course.district}</Badge>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Ultimo Colocado', value: notaCorte.toFixed(1), highlight: true },
              { label: 'Nota Minima', value: String(course.notaMinima), highlight: false },
              { label: 'Pesos Sec/Exam', value: `${(course.pesoSecundario*100).toFixed(0)}/${(course.pesoExame*100).toFixed(0)}`, highlight: false },
            ].map(s => (
                ...
                <div className="mt-1 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-[10px] text-muted-foreground">
                      ({profile.mediaSecundario.toFixed(1)} x {(course.pesoSecundario*100).toFixed(0)}%) + (Exames x {(course.pesoExame*100).toFixed(0)}%)
                    </div>
                  </div>
                  {userGrade >= notaCorte && meetsMinimum ? (
          ...
          <GradeEvolutionChart historico={course.historico} courseName={course.name} />
        </div>
      </DialogContent>
    </Dialog>
  )

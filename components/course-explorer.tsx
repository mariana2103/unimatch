...
function transformCourse(row: any, courseReqs: any[]): CourseUI {
  return {
    ...
    historico: row.history
      ? row.history.map((h: { year: number; nota_f1?: number; nota_f2?: number; nota?: number; vagas_f1?: number; vagas_f2?: number }) => ({
          year:     h.year,
...
export function CourseExplorer({ onCoursesLoaded, onViewDetails }: CourseExplorerProps) {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10 py-6 sm:py-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 9 }).map((_, i) => (
            <div key={i} className="rounded-2xl border border-border/40 bg-card p-4 flex flex-col gap-3">
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10 py-6 sm:py-8 flex flex-col gap-6">
